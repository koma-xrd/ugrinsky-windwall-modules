...
def export_coupon(parameters: DesignParameters, output_dir: Path) -> dict:
    ...
    lift = parameters.bayonet.seating_headroom_mm
    snap = (cq.Workplane('XY').circle(22).circle(20.4)
            .extrude(2+lift).translate((0,0,3.9)).val())

    def motion_intersection(moving):
        collision = moving.intersect(coupon.female).val()
        volume = collision.Volume()
        rigid = collision.cut(snap).Volume() if volume >= .01 else volume
        return {'intersection_mm3': volume, 'rigid_intersection_mm3': rigid}

    motion = [{'travel_deg': min(angle/2,travel),
               **motion_intersection(coupon.male_at_travel(min(angle/2,travel)).translate((0,0,lift)))}
              for angle in range(ceil(travel*2)+1)]
    seating = [{'lift_mm': lift*fraction,
                **motion_intersection(coupon.male.translate((0,0,lift*fraction)))}
               for fraction in (1, .8, .5, .25, 0)]
    insertion = [coupon.male_at_travel(0).translate((0,0,insertion_lift)).intersect(coupon.female).val().Volume()
                 for insertion_lift in range(ceil(lift),25)]
    ...
    if locked >= 0.01 or max(insertion) >= 0.01 or maximum_rigid >= .01 or ccw <= 0.05 or cw <= 0.01:
