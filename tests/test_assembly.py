...
class AssemblyTests(unittest.TestCase):
    ...
    def test_continuous_m8_shaft_clears_all_rotating_and_stationary_parts(self):
        ...
        self.assertAlmostEqual(box.zmax, 499.14, places=5)
    ...
    def test_closure_ties_both_blade_ends_and_uses_existing_pilot_centers(self):
        ...
        for x in (-24,24):
            hole = cq.Workplane('XY').center(x,0).circle(1.6499).extrude(5).translate((0,0,487.84))
            self.assertLess(closure.intersect(hole).val().Volume(), 0.01)
            self.assertTrue(closure.val().isInside((x+3,0,489.84)))
        ...
        for sign in (-1,1):
            half = cq.Workplane('XY').box(100,200,1,centered=(False,True,False)).translate((0 if sign == 1 else -100,0,487.34))
            outside = cq.Workplane('XY').circle(62).circle(36).extrude(1).translate((0,0,487.34))
    ...
    def test_closure_preserves_independent_washer_load_and_upward_service(self):
        ...
        self.assertAlmostEqual(self.assembly.parts['top_washer'].val().BoundingBox().zmin,487.34,places=5)
        self.assertAlmostEqual(self.assembly.parts['top_nut'].val().BoundingBox().zmin,489.34,places=5)
