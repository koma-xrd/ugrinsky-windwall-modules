...
class JointCouponTests(unittest.TestCase):
    ...
    def test_calibration_recess_is_4_point_2_mm_deep_and_has_no_solid_hex_floor(self):
        ...
        self.assertAlmostEqual(male.val().BoundingBox().zmax, 12.7)
        probe = (cq.Workplane('XY').polygon(6, 13.2999/cos(radians(30)))
                 .extrude(4.1999).translate((0,0,8.5001)))
        self.assertLess(male.intersect(probe).val().Volume(), 0.01)
        self.assertTrue(male.val().isInside((5.2,0,8.49)))
        self.assertFalse(male.val().isInside((5.2,0,8.51)))
        self.assertFalse(male.val().isInside((0,6.5,8.49)))
        self.assertTrue(self.interface.male.val().isInside((5.2,0,12.6)))
